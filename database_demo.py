...
from src.models.database import Base, User, ApiKey, ImageAnalysis, AnalysisStatus
...
async def demo_crud_operations(async_session):
    ...
    async with async_session() as session:
        ...
        logger.info(f"✅ Found user: {user.username} with preferences: {user.preferences}")
...
async def demo_relationships(async_session):
    ...
    async with async_session() as session:
        ...
        for user, api_key, analysis in result:
            ...
            logger.info(f"   Analysis: {analysis.status} - Cost: ${analysis.cost_cents/100:.2f}")
...
async def demo_complex_queries(async_session):
    ...
    async with async_session() as session:
        ...
        for row in result:
            logger.info(f"   {row.status}: {row.count} analyses, avg time: {row.avg_time:.1f}ms")
