...
from src.cache.redis_config import redis_config, cache_manager
...
class CachingDemo:
    ...
    async def demo_write_through_pattern(self):
        ...
        start_time = time.time()
        result1 = await analysis_cache.get_or_analyze_image(image_data, fake_analyze_image)
        ...
        start_time = time.time()
        result2 = await analysis_cache.get_or_analyze_image(image_data, fake_analyze_image)
    ...
    async def demo_rate_limiting(self):
        ...
        for i in range(1, 8):  # Try 7 requests (should hit limit)
            ...
            print(f"Request {i}: {status} - Remaining: {result['remaining']}, Count: {result['current_count']}")
