...
class ImageMetadata(BaseModel):
    ...
    color_space: Optional[str] = Field(None, description="Color space (RGB, RGBA, etc.)")
...
class AnalysisRequest(BaseModel):
    ...
    def validate_image_url(cls, v):
        ...
        if v and not str(v).lower().endswith(('.jpg', '.jpeg', '.png', '.bmp', '.gif', '.webp')):
...
class AnalysisResult(BaseModel):
    ...
    confidence_threshold: float = Field(..., ge=0, le=1, description="Applied confidence threshold")
    total_objects_detected: int = Field(..., ge=0, description="Total objects before filtering")
