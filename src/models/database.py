...
class User(BaseModel):
    ...
    # Relationships
    analyses: Mapped[List["ImageAnalysis"]] = relationship("ImageAnalysis", back_populates="user")
...
class ImageAnalysis(BaseModel):
    ...
    image_hash = Column(String(64), nullable=True, index=True)  # SHA-256 for deduplication
    ...
    requested_features = Column(ARRAY(String), nullable=False)  # ["description", "objects", "text"]
    ...
    cost_cents = Column(Integer, default=0, nullable=False)  # Store in cents to avoid float precision issues
...
class AnalysisCache(BaseModel):
    ...
    __table_args__ = (
        UniqueConstraint("image_hash", "features_hash", "model_version", name="uq_cache_key"),
