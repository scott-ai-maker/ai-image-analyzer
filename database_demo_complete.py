...
from src.models.database_compatible import Base, User, ApiKey, ImageAnalysis, AnalysisStatus
        ...
        async with self.async_session() as session:
            ...
            result = await session.execute(
                select(
                    ...
                    func.avg(ImageAnalysis.processing_time_ms).label("avg_processing_time")
                )
            ...
            for row in result:
                ...
                logger.info(f"   {row.username}: {row.analysis_count} analyses, ${total_cost:.2f} total, {avg_time:.0f}ms avg")
    ...
    async def demonstrate_transactions(self):
        ...
        try:
            async with self.async_session() as session:
                ...
                await session.commit()
                logger.info("✅ Transaction 1: Successfully created user with related data")
    ...
    async def demonstrate_complex_queries(self):
        ...
        async with self.async_session() as session:
            ...
            for row in result:
                ...
                if row.avg_time:
                    logger.info(f"      Times: {row.avg_time:.0f}ms avg, {row.min_time}-{row.max_time}ms range")
            ...
            for row in result:
                revenue = row.daily_revenue_cents / 100 if row.daily_revenue_cents else 0
                logger.info(f"   {row.analysis_date}: {row.daily_count} analyses, ${revenue:.2f} revenue")
